...
export default function PlacementTask({ content, answers, onChange, mode }: Props) {
  ...
  const cube = (item: Item, index: number) => {
    ...
    return (
      <Box
        key={item.id}
        draggable
        onDragStart={() => setDrag({ itemId: item.id })}
  ...
  const zone = (section: Section) => {
    ...
    return (
      ...
      >
        ...
        <Stack direction="row" spacing={1} flexWrap="wrap" justifyContent="center" sx={{ mt: 1, width: '100%' }}>
  ...
  const column = (section: Section) => {
    const Icon = zoneIconFor(section.icon ?? section.id)
    const inZone = items.filter((i) => placed.get(i.id) === section.id)
    return (
      <Box key={section.id} sx={{ flex: 1, minWidth: 0 }}>
        <Paper
          onClick={() => placeInto(section)}
          sx={{
            p: 1.5,
  ...
  const tray = (
    <Paper sx={{ p: 2, background: '#FFF8E1', border: '2px solid #FFE082' }}>
      ...
      {pendingItems.length === 0 ? (
        ...
        <Stack direction="row" spacing={1.5} flexWrap="wrap">
          {pendingItems.map((i, idx) => cube(i, items.indexOf(i)))}
        </Stack>
      )}
    </Paper>
  )
