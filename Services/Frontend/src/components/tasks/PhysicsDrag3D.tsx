...
export default function PhysicsDrag3D({
  content,
  answers,
  onChange,
}: TaskComponentProps) {
  ...
  const placedOf = useCallback(
    (itemId: string) => getAnswer(answers, itemId),
    [answers],
  );
  const pending = useMemo(
    () => items.filter((i) => placedOf(i.id) === undefined),
    [items, placedOf],
  );
  ...
  useEffect(() => {
    ...
    const list = pending.filter(
      (i) => !next[i.id] || (!next[i.id].grabbed && !next[i.id].animating),
    );
    ...
    const startX =
      tray.x + (tray.w - firstRowCount * slotW) / 2 + slotW / 2 - HALF;
  ...
  const zoneIdAt = useCallback((cx: number, cy: number): string | undefined => {
    ...
    for (const [zid, el] of zoneRefs.current) {
      ...
      if (
        cx >= z.left - rect.left &&
        cx <= z.right - rect.left &&
        cy >= z.top - rect.top &&
        cy <= z.bottom - rect.top
      ) {
        return zid;
      }
    }
          ...
          <Box sx={{ order: { xs: 1, md: 2 } }}>
            ...
            >
              <Chip
                ...
                label={
                  pending.length === 0
                    ? "Все кубики разложены!"
                    : `Возьми кубик и перетащи в зону (${pending.length} осталось)`
                }
